...
def chat_page():
    ...
    if chat_model is None:
        st.info("🔧 No Groq API key found. Please set GROQ_API_KEY in your environment, then reload the app.")
        return
    ###############################################################################################################
...
def get_chat_response(chat_model, messages, system_prompt: str):
    """Get response from the chat model"""
    try:
        # Prepare messages for the model
        formatted_messages = [SystemMessage(content=system_prompt)]
...
def chat_page():
    ...
    # Default system prompt (we'll customize later)
    system_prompt = ""
...
#################################################################################################################
    ...
    # Chat input (now using RAG + Web Search pipeline)
    if prompt := st.chat_input("Ask something..."):
        ...
        if (
            "embed_client" not in st.session_state
